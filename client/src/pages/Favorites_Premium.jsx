...
const Favorites = () => {
  ...
  const tabs = [
    ...
    { id: 'anime', label: 'Anime', count: favoriteAnime?.length || 0, emoji: '📺' },
    { id: 'manga', label: 'Manga', count: favoriteManga?.length || 0, emoji: '📚' },
    { id: 'characters', label: 'Characters', count: favoriteCharacters?.length || 0, emoji: '👥' },
  ];

  const tabVariants = {
    inactive: { backgroundColor: 'rgba(0, 0, 0, 0.1)', color: 'rgba(255, 255, 255, 0.6)' },
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16">
          ...
          >
            ...
            <p className="text-xl text-text-secondary mb-8 max-w-2xl mx-auto">
              Your carefully curated collection of beloved anime, manga, and characters
            </p>

            {/* Stats Overview */}
            <div className="flex justify-center items-center space-x-8">
              <div className="text-center">
                <div className="text-3xl font-bold text-accent-cyan mb-1">{totalFavorites}</div>
                <div className="text-sm text-text-secondary">Total Favorites</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-accent-magenta mb-1">{favoriteAnime?.length || 0}</div>
                <div className="text-sm text-text-secondary">Anime</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-accent-yellow mb-1">{favoriteManga?.length || 0}</div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
              ...
              >
                <div className="text-red-400 mb-6">
                  ...
                  <p className="text-text-secondary">
                    {error?.message || "There was an error loading your favorites."}
                  </p>
                </div>
              </motion.div>
            )}
            ...
            {currentItems.length === 0 && !loading && !error ? (
              ...
              >
                <div className="text-8xl mb-6">💔</div>
                <h3 className="text-2xl font-semibold mb-4">No {activeTab === 'all' ? 'favorites' : activeTab} yet</h3>
                <p className="text-text-secondary mb-8 max-w-md mx-auto">
                  Start building your collection by adding your favorite {activeTab === 'all' ? 'anime, manga, and characters' : activeTab} to your favorites.
                </p>
                <Link
                  to="/anime"
                  className="bg-gradient-to-r from-accent-cyan to-accent-magenta text-bg-primary px-8 py-3 rounded-lg font-semibold hover:shadow-lg hover:shadow-accent-cyan/25 transition-all duration-300"
                >
                  Discover {activeTab === 'all' ? 'Content' : activeTab === 'characters' ? 'Characters' : activeTab}
                </Link>
              </motion.div>
            ) : (
              <AnimatedGrid 
